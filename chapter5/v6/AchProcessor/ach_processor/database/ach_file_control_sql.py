...
from chapter5.AchProcessor_V4_PreAchRecordRefactor.ach_processor.database.db_utils import (
    get_db_connection,
)
...
class AchFileControlSql:
    ...
    def insert_record(self, ach_file_control: AchFileControlSchema):
        with get_db_connection() as conn:
            conn.execute(
                """
            INSERT INTO ach_file_control_records (ach_records_id, ach_file_headers_id, record_type_code, batch_count, 
            block_count, entry_addenda_count, entry_hash, total_debit_entry_dollar_amount,
            total_credit_entry_dollar_amount, reserved)
            VALUES (%(ach_records_id)s, %(ach_file_headers_id)s, %(record_type_code)s, %(batch_count)s, %(block_count)s,
    ...
    def get_record(self, ach_records_id: UUID) -> AchFileControlSchema:
        with get_db_connection(row_factory=class_row(AchFileControlSchema)) as conn:
            result = conn.execute(
                """
                SELECT * FROM ach_file_control_records WHERE ach_records_id = %s
                """,
                [ach_records_id.hex],
            )
