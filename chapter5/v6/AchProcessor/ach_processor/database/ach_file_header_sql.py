...
class AchFileHeaderSql:
    ...
    def insert_record(self, ach_file_header: AchFileHeaderSchema) -> UUID:
        with get_db_connection() as conn:
            conn.execute(
                """
            INSERT INTO ach_file_headers (ach_records_id,
                           ...
                           VALUES (%(ach_records_id)s, %(record_type_code)s, %(priority_code)s,
    ...
    def get_record(self, ach_records_id: UUID) -> AchFileHeaderSchema:
        with get_db_connection(row_factory=class_row(AchFileHeaderSchema)) as conn:
            result = conn.execute(
                """
                SELECT * FROM ach_file_headers WHERE ach_records_id = %s
                """,
                [ach_records_id.hex],
            )
