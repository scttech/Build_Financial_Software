...
from chapter10.v3.AchParser.ach_processor.ach_file_processor import AchFileProcessor
from chapter10.v3.AchParser.app.main import app
from chapter10.v3.AchParser.tests.ach_processor.sql_utils import SqlUtils
...
@when(
    parsers.parse('I search for transactions with search criteria "{search_criteria}"')
)
def search_request_for_transactions(api_response, search_criteria):
    response = client.get(
        f"/api/v1/files/transactions/search?criteria={search_criteria}"
    )
    assert response.status_code == 200, response.text
    api_response["response"] = response.json()
...
@then(
    parsers.parse(
        'I should receive an error of "{error_code}" and a message of "{error_message}"'
    )
)
...
@then(
    parsers.parse('I should receive an unparsed record of "{expected_unparsed_record}"')
)
def receive_an_unparsed_record(api_response, expected_unparsed_record):
    ...
    assert (
        expected_unparsed_record == actual_unparsed_record
...
def has_exceptions_validation(api_response, expected_value):
    ...
    assert (
        expected_bool_value == actual_exceptions_flag
    ), f"Expected {expected_value} in {api_response['response']}"


@then(
    parsers.parse(
        'I should receive a list of "{transaction_count}" transaction with the amount of "{transaction_amount}"'
    )
)
def has_expected_transaction_and_amount(
    api_response, transaction_count, transaction_amount
):
    assert (
        len(api_response["response"]) == 1
    ), f"Expected {transaction_count} transaction, but got {len(api_response['response'])}"
    response_amount = api_response["response"][0]["amount"]
    assert (
        response_amount == transaction_amount
    ), f"Expected {transaction_amount} in {response_amount}"


@then(
    parsers.parse(
        'I should receive a list of "{transaction_count}" transaction with the individual name of "{individual_name}"'
    )
)
def has_expected_transaction_and_amount(
    api_response, transaction_count, individual_name
):
    assert (
        len(api_response["response"]) == 1
    ), f"Expected {transaction_count} transaction, but got {len(api_response['response'])}"
    ...
    assert (
        response_individual_name == individual_name
    ), f"Expected {individual_name} in {response_individual_name}"
