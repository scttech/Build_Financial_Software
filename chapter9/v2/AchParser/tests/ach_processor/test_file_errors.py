...
def test_incorrect_file_id_modifier(parser):
    try:
        process_file_and_verify_exception(ach_filename="invalid_file_id.ach",
...
def test_incorrect_immediate_destination(parser):
    process_file_and_verify_exception("invalid_immediate_destination.ach", 1,
                                      AchExceptions.INVALID_IMMEDIATE_DESTINATION.value, parser)


def process_file_and_verify_exception(ach_filename: str, expected_exceptions_count: int, expected_exceptions_code: str,
                                      parser: AchFileProcessor):
