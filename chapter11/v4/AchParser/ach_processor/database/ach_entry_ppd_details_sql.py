...
class AchEntryPpdDetailsSql:
    ...
    def insert_record(self, ach_entry_ppd_details: AchEntryPpdDetailsSchema):
        with get_db_connection() as conn:
            conn.execute(
                """
            INSERT INTO ach_ppd_entry_details (ach_records_type_6_id,record_type_code, transaction_code,
    ...
    def get_record(self, ach_records_id: UUID) -> AchEntryPpdDetailsSchema:
        with get_db_connection(row_factory=class_row(AchEntryPpdDetailsSchema)) as conn:
            result = conn.execute(
                """
                SELECT * FROM ach_ppd_entry_details WHERE ach_records_type_6_id = %s
                """,
                [ach_records_id.hex],
            )
